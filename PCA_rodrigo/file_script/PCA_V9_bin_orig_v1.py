...
# Crear el subplot de PCA (2D inicialmente) y graficar al inicio
    ...
    def copy_to_clipboard(event):
        """Copia la imagen completa de la figura al portapapeles."""
        buf = io.BytesIO()
        ...
        image.show()  # Abre la imagen temporalmente
        buf.close()
        print("Imagen copiada al portapapeles. Pégala en Word o PowerPoint.")

    # Crea un botón adicional para copiar al portapapeles
    button_ax_copy = fig.add_axes([0.17, 0.05, 0.1, 0.05])  # [left, bottom, width, height]
...
if __name__ == "__main__":
    manual_offset = 0
    plot_combined_layout(group_directories, group_labels, raman_shift_range, manual_offset, color_order)
