...
def snv_normalization(data):
    ...
    """
    # Convertir las intensidades en una lista de listas (espectros por punto de medición)
    intensities = [point for point in data.values()]

    # Convertir a un DataFrame para facilidad de manejo
    df_intensities = pd.DataFrame(intensities).T  # Transponemos para que cada columna sea un punto de medición

    # Crear el objeto StandardScaler para normalización SNV
    scaler = StandardScaler()

    # Normalizar las intensidades (SNV)
    normalized_data = scaler.fit_transform(df_intensities)

    # Regresamos el resultado en formato de diccionario
    normalized_dict = {}
...
def process_directory_and_save_combined(directory_path, output_file):
    """
    Procesa todos los archivos CSV en una carpeta y guarda los datos combinados en un único archivo binario.
    """
    try:
        ...
        # Guardar los datos combinados en un solo archivo binario
...
def process_single_matrix(df, file_path, combined_data):
    ...
    try:
        ...
        # Asegurarse de que la primera fila contiene los Raman Shifts
        raman_shifts = df.iloc[0].dropna().values.tolist()
        intensity_data = {}

        # Verificar que haya filas suficientes para la matriz
        num_rows = len(df) - 1
        matrix_size = int(num_rows ** 0.5)
        if matrix_size ** 2 != num_rows:
            raise ValueError(f"El archivo no corresponde a una matriz cuadrada válida. El tamaño debería ser {matrix_size}x{matrix_size}.")

        # Recorrer el resto del DataFrame (que son los puntos de medición)
        for i, row in df.iloc[1:].iterrows():
            m, n = divmod(i, matrix_size)
            point_name = f"Point_{m+1}_{n+1}"
            intensity_data[point_name] = row.dropna().values.tolist()

        # Normalizar los datos de intensidad utilizando SNV
...
if __name__ == "__main__":
    directory_path = "./"  # Cambia por el directorio donde están tus archivos CSV
    output_file = "snv_combined_data_spectra.bin"  # Nombre del archivo binario combinado
    process_directory_and_save_combined(directory_path, output_file)
